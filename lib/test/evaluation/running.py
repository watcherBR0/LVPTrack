...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k', 'got10k_train_haze']:
    ...
    for key, data in output.items():
        ...
        #0715
        if key == 'target_bbox':
            if seq.dataset == 'got10k_train_haze':
    ...
    def _results_exist():
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k']:
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}.txt'.format(base_results_path)
            elif seq.dataset == 'got10k_train_haze':
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}/pl.txt'.format(base_results_path)
            else:
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8, pl_produce=False):
    global VISDOM
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                ...
                tracker = Tracker(tracker_name, tracker_param, dataset_name, run_id, ep_id, debug=debug, pl_produce=pl_produce)
