...
class BaseTrainer:
    ...
    def update_net_extreme_params(self, keep_rate=0.9996):
        ...
        net_extreme = self.actor.net_extreme.module if multigpu.is_multi_gpu(self.actor.net_extreme) else self.actor.net_extreme
        ...
        for i in range(num_tries):
            # try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    #0710
                    # add by siyuan, during prompt training, we do not need EMA
                    # if self.epoch == 1:
                    #     self.update_net_extreme_params(keep_rate=0.)
                    # elif self.epoch % 5 == 0:
                    #     self.update_net_extreme_params(keep_rate=0.9)
                    self.update_net_extreme_params(keep_rate=0.)
                    ...
                    save_every_epoch = getattr(self.settings, "save_every_epoch", False)
                    save_epochs = []
                    if save_every_epoch or (epoch % 20 == 0 and epoch >= 100) or epoch in save_epochs\
                     or (epoch % 10 == 0 and epoch >= 200):
                        # or epoch >= int(max_epochs*0.8)-1:
                    # if epoch > (max_epochs - 10) or save_every_epoch or epoch % 100 == 0:
                        if self._checkpoint_dir:
                            if self.settings.local_rank in [-1, 0]:
                                #0712
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        if checkpoint is None:
            ...
            checkpoint_list_extreme = sorted(glob.glob('{}/{}/{}_extreme_ep*.pth.tar'.format(self._checkpoint_dir,
                                                                             self.settings.project_path, net_extreme_type)))
            if checkpoint_list:
