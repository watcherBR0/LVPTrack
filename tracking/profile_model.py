...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='lightUAV', choices=['lightUAV'], help='training script name')
    parser.add_argument('--config', type=str, default='vit_256_ep300_0405', help='yaml configure file name')
...
def evaluate_vit(model, template, search):
    ...
    with torch.autocast(device.type, enabled=use_fp16):
        macs1, params1 = profile(model, inputs=(template, search, template_bb, mode),
    ...
    with torch.autocast(device.type, enabled=use_fp16):
        with torch.no_grad():
            # overall
            for i in range(T_w):
                _ = model_(template, search, template_bb, mode)
            start = time.time()
            for i in range(T_t):
                _ = model_(template, search, template_bb, mode)
            torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "lightUAV":
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_lightUAV
